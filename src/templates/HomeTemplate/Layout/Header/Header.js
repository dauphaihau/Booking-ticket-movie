...
function Header(props) {
    ...
    const handleChange = (values) => {
        let keyLang = '';
        values === true ? keyLang = 'vi' : keyLang = 'en';
    ...
    return (
        <header className=" bg-gray-100  opacity-70 bg-black text-white fixed w-full z-10">
            <div className="container flex justify-between h-16 mx-auto">
                ...
                <ul className="items-stretch hidden space-x-3 lg:flex">
                    <li className="flex">
                        <NavLink exact to={'/home'} activeClassName='borderBottom'
                                 className=" flex i text-white items-center px-4 -mb-1 border-b-2 border-transparent text-violet-600 border-violet-600">{t('Home')}</NavLink>
                    </li>
                    <li className="flex">
                        <NavLink exact to={'/contact'} activeClassName='borderBottom'
                                 className="flex text-white items-center px-4 -mb-1 border-b-2 border-transparent text-violet-600 border-violet-600">{t('Contact')}</NavLink>
                    </li>
                    <li className="flex">
                        <NavLink exact to='/news' activeClassName='borderBottom'
                                 className="flex text-white items-center px-4 -mb-1 border-b-2 border-transparent text-violet-600 border-violet-600">{t('News')}</NavLink>
                    </li>
                </ul>
                ...
                <Switch onChange={handleChange} className='mt-5' style={{marginTop: 23}} checkedChildren="VI"
                        unCheckedChildren="VI" defaultChecked={false}/>
            </div>
        </header>
    );
}
